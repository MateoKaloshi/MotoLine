// Router API therrasin nje funksion te caktuar nga controlleri perkates
  ...
  try {
      ...
      for (const field of candidateFields) {
        ...
        if (docs && docs.length > 0) {
          images = docs
            .map((d) => {
              if (d.url) {
                const url = d.url.startsWith("http")
                  ? d.url
                  : `${hostPrefix}${d.url.startsWith("/") ? "" : "/"}${d.url}`;
                return {
                  url,
              ...
              if (d.path) {
                ...
                return {
                  url: `${hostPrefix}/uploads/${filename}`,
                  filename,
                  _id: d._id || null,
                };
              }
              if (d.filename) {
                return {
                  url: `${hostPrefix}/uploads/${d.filename}`,
                  filename: d.filename,
                  _id: d._id || null,
                };
              }
    ...
    if (!images.length) {
      ...
      if (collections.find((c) => c.name === "images")) {
        ...
        const oid = mongoose.Types.ObjectId.isValid(bikeId)
    ...
    if (!images.length) {
      try {
        if (fs.existsSync(uploadDir)) {
          ...
          if (matched.length > 0) {
            images = matched.map((f) => ({
              url: `${hostPrefix}/uploads/${f}`,
              filename: f,
              _id: null,
            }));
          }
    ...
    images = images.filter(Boolean).map((i) => ({
      url: i.url,
