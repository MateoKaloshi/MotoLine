...
exports.login = async (req, res) => {
  try {
    ...
    if (!user.password) {
      ...
      return res
        .status(500)
        .json({ message: "User password not set. Contact support." });
    }
    ...
    try {
      ...
      console.error(
        ...
        bcryptErr && bcryptErr.stack ? bcryptErr.stack : bcryptErr
      );
    ...
    try {
      ...
      console.error(
        "jwt.sign error:",
        jwtErr && jwtErr.stack ? jwtErr.stack : jwtErr
      );
...
exports.register = async (req, res) => {
  try {
    const { first_name, last_name, password, email, phone_number, adress } =
      req.body;
